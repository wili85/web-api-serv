...
class Api{
	...
	function getAseguradoById($item){
		/*
		if (!doAuthenticate()) {
			$this->error('No hay autentifican');
		}
		*/
		include '../model/Beneficiario.php';
		$a = new Afiliado();
		...
		if ($nr > 0) {
			if (isset($rs['Error'])) {
				...
				for ($i = 0; $i < $nr; $i++) {
					...
					$tipodocidentidad = '1';	//($p[40]=='DNI')?1:2;

					$dataRemitente["ICODREMITENTE1"] = '0';
					$dataRemitente["TIPOPERSONA"] = '1';	$tipodocidentidad;
					$dataRemitente["NOMBRE"] = $rs[$i]['nombrepaciente']; //$p[42];
					$dataRemitente["NUMDOCUMENTO"] = $rs[$i]['numdocpaciente']; //$p[41];
					$dataRemitente["DIRECCION"] = '';
					$dataRemitente["DEPARTAMENTO"] = '';
					$dataRemitente["PROVINCIA"] = '';
					$dataRemitente["DISTRITO"] = '';
					...
					$dataSolicitud["casunto1"] = 'SOLICITUD DE REMBOLSO';
					$dataSolicitud["cobservaciones1"] = 'SOLICITUD DE REMBOLSO';
					$dataSolicitud["nnumfolio1"] = null;		//$p[48];//$this->input->post('nroFolioHT');
					$dataSolicitud["nflgenvio1"] = '1';
					$dataSolicitud["ffecregistro1"] = null;
					$dataSolicitud["nflgestado1"] = '5';
					$dataSolicitud["nflganulado1"] = null;
		...
		if ($nr > 0) {
			if (isset($rs['Error'])) {
				...
				for ($i = 0; $i < $nr; $i++) {
					...
					$reembolso[$i]['rutarecetavale'] = $rs[$i]['rurutarecetavaletarecetavale'];
				}
		...
		if ($nr > 0) {
			if (isset($rs['Error'])) {
				...
				for ($i = 0; $i < $nr; $i++) {
					...
					$reembolso[$i]['nomservicio'] = $rs[$i]['nomservicio'];
					$reembolso[$i]['codupss'] = $rs[$i]['codupss'];
					$reembolso[$i]['rutarecetavale'] = $rs[$i]['rutarecetavale'];
				}
